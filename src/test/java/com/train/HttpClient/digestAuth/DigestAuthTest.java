...
public class DigestAuthTest extends BaseHttpClient {

	static String appAddr = "http://localhost:8080";
	static String docUri = "/DigestAuth/doc/index.html";
	static String appUri = "/DigestAuth/app/index.html";

	static String username = "tomcat";
	static String password = "tomcat";
	static String realm = "www.rainbow954-digest.cn";

	private String getDigestPassword(String username, String userPassword,
			String realm) {
		String key = MD5Object.encrypt(username + ":" + realm + ":" + userPassword);
		
		PasswordDigest dig = new PasswordDigest();
		String pass = null;
		try {
			...
			for (int i = 0; i < password.length; i++)
			 ...
			 System.out.println(key +":"+MD5Object.byteToString(md5code));
			 pass = (new BASE64Encoder()).encodeBuffer(md5code);
			
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
		String p = dig.getPasswordDigest(username, userPassword, realm);
	...
	public void testMd5Base64(){
		//getDigestPassword("tomcat", "tomcat", realm);
		//getDigestPassword("root", "123456", realm);
		getDigestPassword("mysql", "mysql", realm);
	}
	@Test
	public void testDigestAuthDoc() throws Exception {
		
		testDigestAuth("tomcat", "tomcat", appAddr, docUri);
	}

	
	public void testDigestAuthAppFail() throws Exception {
		testDigestAuth("tomcat", "tomcat", appAddr, appUri);
	}

	
	public void testDigestAuthAppSucc() throws Exception {

		testDigestAuth("root", "123456", appAddr, appUri);
	}

	public void testDigestAuth(String username, String password,
